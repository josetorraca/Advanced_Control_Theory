    ...
    def optimize_dyn(self, xf, thetaf=[], ksim=None):
        ...
        for i in range(0, self.u.shape[0]):
            uopt[:, i] = wopt[self.x.shape[0]+self.x.shape[0]*self.m+i::self.x.shape[0]+
                              self.x.shape[0]*self.m+self.u.shape[0]].reshape(-1) #optimal inputs
        return {
            'x': xopt, 
            'u': uopt
            }    
...
class LSE:
    ...
    def __init__(self, F, R, x, y, u, theta, thetaguess=None, lbtheta=None, 
        ...
        J = (self.y - vcat(self.rfsolver(self.x0, self.u, self.theta))
             ...
             self.u, self.theta))[:self.x.shape[0]]) #quadratic error cost function
...
class NMPC:
    ...
    def __init__(self, dt, N, M, Q, R, W, x, u, c, dx, theta, xguess=None, 
            ...
            ubdu=None, m=3, pol='legendre', tgt=False, DRTO=False, opts={}):   
        self.dt = dt
        self.dx = dx
        self.x = x
        self.c = c
